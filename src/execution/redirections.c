...
/* Applique toutes les redirections pour une seule commande. */
...
/* Gère une redirection here-document (<<) en utilisant un pipe. */
static int	handle_heredoc(const char *delimiter)
{
	int	pipefd[2];
...
/* Lit l'entrée utilisateur pour le here-document et l'écrit dans le pipe. */
static void	read_heredoc_input(int write_fd, const char *delimiter)
{
	char	*line;
